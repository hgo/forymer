 ...
 */
@Entity
public class GeoLocation extends Model implements Cloneable {

    public static final double RADIUS = 6371.01d;
    ...
    public double distanceTo(GeoLocation location, double radius) {
        return Math.acos(Math.sin(radLat) * Math.sin(location.radLat) + Math.cos(radLat) * Math.cos(location.radLat) * Math.cos(radLon - location.radLon))
                * radius;
    }
